...
cat > $OUTDIR/tmp9.mainstream

#  input = tmp8.pairs.x.sql
# output = tmp9.pairs.x.sql
...
# location of the AWK script reduce_pairs_spaces.awk 
# N.B. couldn't figure it out with /proc/self etc. so correct the hardcoded
# version when necessary.
SCRIPTDIR=../scripts/step.reduce_pairs_spaces/

# doesn't need x flag, awk will interpret it.
if [ ! -d $SCRIPTDIR ] || [ ! -f $SCRIPTDIR/reduce_pairs_spaces.awk ]; then
  eecho "ERROR SCRIPTDIR not correct or reduce_pairs_spaces.awk program not found"
  exit 1
fi


msg=$( wc -l tmp8.pairs.?.sql 2>&1 )
eecho "$msg"
...
for dist in 1 ; do
  ...
  intable=$OUTDIR/tmp8.pairs.$dist.sql
#DBG# intable=ananasjuice.sample
  ...
  for li in $( seq 1 $nlinesbefore ); do
    ...
    bingo=$( echo "$shortstring""""$longstring" | awk -F'' -f $SCRIPTDIR/reduce_pairs_spaces.awk )
...
for dist in 1    ; do
  eecho
  eecho $( mv -v $OUTDIR/tmp.work.$dist $OUTDIR/tmp9.pairs.$dist.sql )
  eecho $( mv -v $OUTDIR/tmp.discard.$dist $OUTDIR/tmp9.pairs.discard.$dist.sql )
  eecho $( wc -l $OUTDIR/tmp8.pairs.$dist.sql )
  eecho $( wc -l $OUTDIR/tmp9.pairs.$dist.sql $OUTDIR/tmp9.pairs.discard.$dist.sql )
done

cat $OUTDIR/tmp9.mainstream
rm $OUTDIR/tmp9.mainstream
